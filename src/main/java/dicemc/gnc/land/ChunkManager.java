...
public class ChunkManager {
	...
	public String tempClaim(ChunkPos ck, UUID player) {
		...
		double balP = AccountUtils.getBalance(player);
		if (balP >= cap.get(ck).price*Config.TEMPCLAIM_RATE.get()) {
			AccountUtils.changeBalance(player, (-1 * (cap.get(ck).price * Config.TEMPCLAIM_RATE.get())));
	...
	public String guildClaim(ChunkPos ck, UUID guild) {
		...
		double balG = AccountUtils.getBalance(guild);
		if (balG >= cap.get(ck).price + outpostFee) {
			AccountUtils.changeBalance(guild, (-1 * (cap.get(ck).price)));
			if (!cap.get(ck).renter.equals(GnC.NIL)) {
				AccountUtils.changeBalance(cap.get(ck).renter, cap.get(ck).price*Config.TEMPCLAIM_RATE.get());
	...
	public String extendClaim(ChunkPos ck, UUID player) {
		...
		if (balP >= cost) {
			AccountUtils.changeBalance(player, (-1 * cost));
